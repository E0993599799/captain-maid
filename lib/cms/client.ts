...
class CMSClient {
  ...
  async restGet<T>(collection: string, params: Record<string, unknown> = {}, options: RequestOptions = {}): Promise<T> {
    ...
    const appendParam = (key: string, value: unknown) => {
      ...
      if (typeof value === "object") {
        return Object.entries(value as Record<string, unknown>).forEach(([childKey, childValue]) => appendParam(`${key}[${childKey}]`, childValue));
      }
  ...
  async getProducts(filters: { locale?: Locale; limit?: number; page?: number } = {}, options: RequestOptions = {}) {
    return this.restGet("products", {
      ...
      page: filters.page || 1,
      sort: "-createdAt",
    }, { next: { tags: ["products"] }, ...options });
  }

  async getProduct(slug: string, locale: Locale = "th", options: RequestOptions = {}) {
    return this.restGet("products", {
      where: { slug: { equals: slug }, contentStatus: { equals: "approved" } },
      locale,
      fallbackLocale: "en",
      depth: 2,
      limit: 1,
